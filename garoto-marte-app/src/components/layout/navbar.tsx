            ...
            <nav className="flex items-center justify-between md:px-16 w-full" aria-label="Global">
                ...
                <div className="flex md:hidden ml-auto">
                    ...
                    >
                        ...
                        <Menu className="h-24 w-24" aria-hidden="true" />
                    </Button>
                </div>
                ...
                <div className="hidden md:flex md:flex-1 md:items-center">
                    ...
                    {navigationLeft.map((item) => (
                        <Link
                            key={item.name}
                            href={item.href}
                            className="text-lg leading-6 text-white hover:text-gray-300 px-3 py-2 transition-colors relative group mr-4"
                        >
                            {item.name}
                ...
                <div className="md:hidden fixed inset-0 z-50 bg-black/70 backdrop-blur-sm">
                    <div className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-black/95 backdrop-blur-md px-6 py-6 sm:max-w-sm border-l border-gray-800">
                        <div className="flex items-center justify-between">
                            ...
                            <Button
                                variant="ghost"
                                className="-m-2.5 rounded-md p-4 text-white text-lg flex items-center justify-center hover:bg-gray-800/50"
                                onClick={() => setMobileMenuOpen(false)}
                            >
                                <span className="sr-only">Close menu</span>
                                <X className="h-24 w-24" aria-hidden="true" />
                            </Button>
                        </div>

                        <div className="mt-10 flow-root">
                            <div style={{ fontFamily: 'GarotoMarteFont, sans-serif', letterSpacing: '0.05em' }} className="text-center text-2xl my-6">
                                GAROTO MARTE
                            </div>
